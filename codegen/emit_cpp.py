...
def _c_params(op: Op) -> str:
    return ", ".join(_C_DECLS[p.kind].format(n=p.name) for p in op.params)


def _call_args(op: Op) -> list[str]:
    args = []
    for p in op.params:
        if p.kind == TENSOR:
...
def emit_header(shard: str, ops: list[Op]) -> str:
    needs_bool = any(p.kind == BOOL for op in ops for p in op.params)
    ...
    lines += [
        "/* Generated ops ({}). Each returns a new tr_tensor handle".format(
            shard
        ),
    ...
    for op in ops:
        lines.append(f"tr_tensor* {op.c_name}({_c_params(op)});")
