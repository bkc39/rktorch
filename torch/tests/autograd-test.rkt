...
(module+ test
  ...
  (test-case "in-place zero!, mul! and copy!"
    ...
    (addcmul! t (tensor '(1.0 2.0)) (tensor '(3.0 4.0)) 0.5)
    (check-equal? (tensor->list t) '(6.5 11.0))
    (addcdiv! t (tensor '(1.0 2.0)) (tensor '(2.0 4.0)) -1.0)
    (check-equal? (tensor->list t) '(6.0 10.5))
    (lerp! t (tensor '(0.0 0.0)) (full 0.5))
    (check-equal? (tensor->list t) '(3.0 5.25))
    (lerp! t t (full 0.25))
    (check-equal? (tensor->list t) '(3.0 5.25) "lerp onto itself is the identity")))
