;; @generated by codegen (python3 -m codegen); DO NOT EDIT.
;; Schema: codegen/aten/native_functions.yaml (libtorch 2.9.0).
;; Parity manifest: (racket-name python-attr (arg kinds ...) inplace?).
;; inplace? is #t for ops checked against torch.Tensor.<attr>_ on a
;; cloned receiver. Consumed by torch/tests/generated-parity-test.rkt.
(
 (abs-tensor "abs" (tensor) #f)
 (adaptive-avg-pool2d "adaptive_avg_pool2d" (tensor int-array) #f)
 (add-tensor! "add_.Tensor" (tensor tensor scalar) #t)
 (addcdiv! "addcdiv_" (tensor tensor tensor scalar) #t)
 (addcmul! "addcmul_" (tensor tensor tensor scalar) #t)
 (avg-pool2d "avg_pool2d" (tensor int-array int-array int-array bool bool optional-int64) #f)
 (batch-norm "batch_norm" (tensor optional-tensor optional-tensor optional-tensor optional-tensor bool double double bool) #f)
 (binary-cross-entropy-with-logits "binary_cross_entropy_with_logits" (tensor tensor optional-tensor optional-tensor int64) #f)
 (broadcast-to "broadcast_to" (tensor int-array) #f)
 (cat "cat" (tensor-list int64) #f)
 (clamp "clamp" (tensor optional-scalar optional-scalar) #f)
 (conv-transpose2d-input "conv_transpose2d.input" (tensor tensor optional-tensor int-array int-array int-array int64 int-array) #f)
 (conv1d "conv1d" (tensor tensor optional-tensor int-array int-array int-array int64) #f)
 (conv2d "conv2d" (tensor tensor optional-tensor int-array int-array int-array int64) #f)
 (copy! "copy_" (tensor tensor bool) #t)
 (cos-tensor "cos" (tensor) #f)
 (cross-entropy-loss "cross_entropy_loss" (tensor tensor optional-tensor int64 int64 double) #f)
 (ctc-loss-intlist "ctc_loss.IntList" (tensor tensor int-array int-array int64 int64 bool) #f)
 (dot "dot" (tensor tensor) #f)
 (dropout "dropout" (tensor double bool) #f)
 (embedding "embedding" (tensor tensor int64 bool bool) #f)
 (eq-scalar "eq.Scalar" (tensor scalar) #f)
 (eq-tensor "eq.Tensor" (tensor tensor) #f)
 (fill-scalar! "fill_.Scalar" (tensor scalar) #t)
 (flip "flip" (tensor int-array) #f)
 (gather "gather" (tensor int64 tensor bool) #f)
 (ge-scalar "ge.Scalar" (tensor scalar) #f)
 (ge-tensor "ge.Tensor" (tensor tensor) #f)
 (group-norm "group_norm" (tensor int64 optional-tensor optional-tensor double bool) #f)
 (gt-scalar "gt.Scalar" (tensor scalar) #f)
 (gt-tensor "gt.Tensor" (tensor tensor) #f)
 (huber-loss "huber_loss" (tensor tensor int64 double) #f)
 (index-add! "index_add_" (tensor int64 tensor tensor scalar) #t)
 (index-copy! "index_copy_" (tensor int64 tensor tensor) #t)
 (index-fill-int-scalar! "index_fill_.int_Scalar" (tensor int64 tensor scalar) #t)
 (index-fill-int-tensor! "index_fill_.int_Tensor" (tensor int64 tensor tensor) #t)
 (index-select "index_select" (tensor int64 tensor) #f)
 (l1-loss "l1_loss" (tensor tensor int64) #f)
 (layer-norm "layer_norm" (tensor int-array optional-tensor optional-tensor double bool) #f)
 (le-scalar "le.Scalar" (tensor scalar) #f)
 (le-tensor "le.Tensor" (tensor tensor) #f)
 (leaky-relu "leaky_relu" (tensor scalar) #f)
 (lerp-tensor! "lerp_.Tensor" (tensor tensor tensor) #t)
 (lt-scalar "lt.Scalar" (tensor scalar) #f)
 (lt-tensor "lt.Tensor" (tensor tensor) #f)
 (masked-fill-scalar "masked_fill.Scalar" (tensor tensor scalar) #f)
 (masked-fill-scalar! "masked_fill_.Scalar" (tensor tensor scalar) #t)
 (masked-fill-tensor! "masked_fill_.Tensor" (tensor tensor tensor) #t)
 (masked-scatter! "masked_scatter_" (tensor tensor tensor) #t)
 (masked-select "masked_select" (tensor tensor) #f)
 (matmul "matmul" (tensor tensor) #f)
 (max-pool2d "max_pool2d" (tensor int-array int-array int-array int-array bool) #f)
 (mean-dim "mean.dim" (tensor optional-int-array bool optional-dtype) #f)
 (mm "mm" (tensor tensor) #f)
 (mul-tensor! "mul_.Tensor" (tensor tensor) #t)
 (mv "mv" (tensor tensor) #f)
 (narrow "narrow" (tensor int64 int64 int64) #f)
 (ne-scalar "ne.Scalar" (tensor scalar) #f)
 (ne-tensor "ne.Tensor" (tensor tensor) #f)
 (nll-loss "nll_loss" (tensor tensor optional-tensor int64 int64) #f)
 (nonzero "nonzero" (tensor) #f)
 (repeat-interleave-self-int "repeat_interleave.self_int" (tensor int64 optional-int64 optional-int64) #f)
 (reshape "reshape" (tensor int-array) #f)
 (scatter-add! "scatter_add_" (tensor int64 tensor tensor) #t)
 (scatter-src! "scatter_.src" (tensor int64 tensor tensor) #t)
 (scatter-value! "scatter_.value" (tensor int64 tensor scalar) #t)
 (select-int "select.int" (tensor int64 int64) #f)
 (silu "silu" (tensor) #f)
 (sin-tensor "sin" (tensor) #f)
 (slice-tensor "slice.Tensor" (tensor int64 optional-int64 optional-int64 int64) #f)
 (sum-dim-intlist "sum.dim_IntList" (tensor optional-int-array bool optional-dtype) #f)
 (take "take" (tensor tensor) #f)
 (take-along-dim "take_along_dim" (tensor tensor optional-int64) #f)
 (tril "tril" (tensor int64) #f)
 (triu "triu" (tensor int64) #f)
 (where-scalar "where.Scalar" (tensor scalar scalar) #f)
 (where-scalarother "where.ScalarOther" (tensor tensor scalar) #f)
 (where-scalarself "where.ScalarSelf" (tensor scalar tensor) #f)
 (where-self "where.self" (tensor tensor tensor) #f)
)
