...
(module+ test
  ...
  (test-case "a non-floating buffer keeps its dtype under a layer dtype move"
    (define-layer Counted (w steps keep)
      #:init ()
      ...
      #:forward (x) (mul x w))
    (define m (Counted))
    (check-equal? (map tensor-dtype (buffers m)) '(int64 bool))
    (to m 'float64)
    (check-equal? (map tensor-dtype (parameters m)) '(float64))
    (check-equal? (map tensor-dtype (buffers m)) '(int64 bool)
                  "as Module.to: dtype only reaches floating-point tensors")
    (check-equal? (tensor->list (car (buffers m))) '(0 1))
    (to m 'cpu 'float32)
    (check-equal? (map tensor-dtype (parameters m)) '(float32))
    (check-equal? (map tensor-dtype (buffers m)) '(int64 bool)))
  ...
  (define (check-device-moves dev dev-name)
    ...
    (let ()
      (define-layer Counted (steps)
        #:init () (set! steps (Buffer (tensor '(0 1))))
        #:forward (x) x)
      (define c (to (Counted) dev 'float64))
      (check-equal? (tensor-device (car (buffers c))) dev
                    "a non-floating buffer still changes device")
      (check-equal? (tensor-dtype (car (buffers c))) 'int64))
    (settle!)
    ...
    (define x (ones 1 1024 #:device dev))
    (check-equal? (tensor-device x) dev "placed at construction")
    (check-equal? (tensor-device (zeros-like x)) dev "zeros-like follows")
    (define y (m x))
