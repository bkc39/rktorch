...
/* Shape manipulation. Each returns a new tr_tensor handle (NULL on error).
 * The new handle may share storage with the input (these are views in ATen
 * where possible); each tr_tensor owns its own reference, so freeing either
 * handle in either order is safe. */

tr_tensor* tr_reshape(const tr_tensor* t, const int64_t* dims, int64_t ndim);

/* Like reshape but requires compatible (contiguous) strides, as torch view. */
...
/* Drop every size-1 dimension. */
tr_tensor* tr_squeeze(const tr_tensor* t);

/* Drop dimension `dim` if it has size 1. */
...
/* Stack `n` same-shaped tensors along a new dimension `dim`. */
